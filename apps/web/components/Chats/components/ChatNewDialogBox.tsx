...
function ChatNewDialogBox() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      dispatch(
        loadinghandler({ isLoading: true, message: "Create a new chat" })
      );
      ...
      setError(("Unable to create chat" + error) as string);
      console.error("Unable to create a chats :: ", error);
      return;
    }
    ...
    "
      ...
      <DialogContent>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter>
            {error && (
              <div className="text-red-600 opacity-50 text-sm">{error}</div>
            )}
